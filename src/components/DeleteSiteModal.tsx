...
import { AlertOctagon, ShieldCheck, XCircle, X } from 'lucide-react';
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden border-2 border-red-500">
        <div className="bg-red-600 text-white px-5 py-3.5 flex items-center justify-between">
          <h4 className="font-bold text-sm sm:text-base flex items-center gap-2">
            ...
            <span>SECURITY CHECK: Delete Project Site</span>
          </h4>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleConfirm} className="p-5 space-y-4">
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-center text-xs font-bold text-red-900">
            <div>PERMANENTLY DELETE ENTIRE SITE:</div>
            <div className="text-sm font-black text-neutral-900 mt-1">{siteName}</div>
          </div>

          <p className="text-gray-600 text-xs leading-relaxed">
            This action will completely remove this project location across all date ranges.{' '}
